...
cv::Rect get_rect(int imgcols,int imgrows, float bbox[4], int nInPutModelW , int nInPutModelH ) {
    int l, r, t, b;
    float r_w = nInPutModelW / (imgcols * 1.0);
    float r_h = nInPutModelH / (imgrows * 1.0);
    if (r_h > r_w) {
        l = bbox[0] - bbox[2] / 2.f;
        r = bbox[0] + bbox[2] / 2.f;
        t = bbox[1] - bbox[3] / 2.f - (nInPutModelH - r_w * imgrows) / 2;
        b = bbox[1] + bbox[3] / 2.f - (nInPutModelH - r_w * imgrows) / 2;
        l = l / r_w;
        r = r / r_w;
        t = t / r_w;
        b = b / r_w;
    } else {
        l = bbox[0] - bbox[2] / 2.f - (nInPutModelW - r_h * imgcols) / 2;
        r = bbox[0] + bbox[2] / 2.f - (nInPutModelW - r_h * imgcols) / 2;
        t = bbox[1] - bbox[3] / 2.f;
        b = bbox[1] + bbox[3] / 2.f;
        l = l / r_h;
        r = r / r_h;
...
void nms(std::vector<Yolo::Detection>& res, float *output, float conf_thresh, float nms_thresh = 0.5,int nMaxOutPutBoxCounts=1000) {
    int det_size = sizeof(Yolo::Detection) / sizeof(float);
    std::map<float, std::vector<Yolo::Detection>> m;
    for (int i = 0; i < output[0] && i < nMaxOutPutBoxCounts; i++) 
	{
...
ILayer* focus(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, 
	          ITensor& input, int inch, int outch, int ksize, std::string lname,
	         int nInPutModelW , int nInPutModelH) {
    ISliceLayer *s1 = network->addSlice(input, Dims3{ 0, 0, 0 }, Dims3{ inch, nInPutModelH / 2, nInPutModelW / 2 }, Dims3{ 1, 2, 2 });
    ISliceLayer *s2 = network->addSlice(input, Dims3{ 0, 1, 0 }, Dims3{ inch, nInPutModelH / 2, nInPutModelW / 2 }, Dims3{ 1, 2, 2 });
    ISliceLayer *s3 = network->addSlice(input, Dims3{ 0, 0, 1 }, Dims3{ inch, nInPutModelH / 2, nInPutModelW / 2 }, Dims3{ 1, 2, 2 });
    ISliceLayer *s4 = network->addSlice(input, Dims3{ 0, 1, 1 }, Dims3{ inch, nInPutModelH / 2, nInPutModelW / 2 }, Dims3{ 1, 2, 2 });
...
IPluginV2Layer* addYoLoLayer(INetworkDefinition *network, std::map<std::string, Weights>& weightMap,
	                         ...
	                         int nInPutModelW, int nInPutModelH,int nClassNums, int nMaxOutPutBoxCounts ) {
    auto creator = getPluginRegistry()->getPluginCreator("YoloLayer_TRT", "1");
    auto anchors = getAnchors(weightMap, lname);
    PluginField plugin_fields[2];
    int netinfo[4] = { nClassNums, nInPutModelW, nInPutModelH, nMaxOutPutBoxCounts };
    ...
    for (size_t i = 0; i < anchors.size(); i++) {
        ...
        kernel.width = nInPutModelW / scale;
        kernel.height = nInPutModelH / scale;
