...
string Tchar2StringC(TCHAR *pStr)
{
...
LPCWSTR String2LpcwstrC(std::string strOrig)
{
...
void Tchar2CharC(const TCHAR * tchar, char * _char)
{
	int iLength;
	//获取字节长度  
...
void doInferenceClassification(IExecutionContext& context, cudaStream_t& stream, float*input, void **buffers, float* output, int batchSize, int nInPutModelW, int nInPutModelH, int nOutPutSize, bool bIsDbInput)
{
	if (bIsDbInput)
	{
...
AIClassifierTool::AIClassifierTool()
{
...
bool AIClassifierTool::Init(tagAIBaseParam* pBaseParam)
{
	//1.1读取配置文件 待添加
	TCHAR exeFullPath[MAX_PATH]; // Full path
	std::string strPath = "";
	GetModuleFileName(NULL, exeFullPath, MAX_PATH); //获取带有可执行文件名路径
	strPath = Tchar2StringC(exeFullPath);
	...
	int nDLTemp = GetPrivateProfileInt(L"DL", L"DLTpye", 0, String2LpcwstrC(strIniPath));
	//test
	pBaseParam->m_AIBuildToolType = AIBuildToolType(nDLTemp); //dpClassifierTool;

	TCHAR szValue[MAX_PATH] = L"";
	char chInputName[MAX_PATH] = { 0 };
	GetPrivateProfileString(L"DL", L"InputName", L"input", szValue, MAX_PATH, String2LpcwstrC(strIniPath));
	...
	GetPrivateProfileString(L"DL", L"OutputName", L"output", szValue, MAX_PATH, String2LpcwstrC(strIniPath));
	...
	GetPrivateProfileString(L"DL", L"ModelPath", L"D:\model\moqie0712_s.engine", szValue, MAX_PATH, String2LpcwstrC(strIniPath));
	...
	nDLTemp = GetPrivateProfileInt(L"DL", L"ClassNum", 0, String2LpcwstrC(strIniPath));
	pBaseParam->m_AICommonParam.nInClassNum = nDLTemp;   //11

	int nModelW = GetPrivateProfileInt(L"DL", L"ModelW", 285, String2LpcwstrC(strIniPath));
	pBaseParam->m_AICommonParam.nInModelInputH = nModelW;           // 285;

	int nModelH = GetPrivateProfileInt(L"DL", L"ModelH", 285, String2LpcwstrC(strIniPath));
...
bool AIClassifierTool::SetParam(tagAIBaseParam*pBaseParam)
{
	//2.1设置内存、显存分配
...
bool AIClassifierTool::PreProcess(tagAIBaseParam*pBaseParam, tagAIBaseResultParam* pBaseResultParam, vector<DEF>vecInDefImg, vector<REF>vecInRefImg)
{
	pBaseParam = &m_tagAIBaseParams;
	if (pBaseParam == NULL)
	{
...
bool AIClassifierTool::Execute(tagAIBaseParam*pBaseParam, tagAIBaseResultParam* pBaseResultParam)
{
	...
	if (pBaseParam->m_AIBuildToolType == dpClassifierTool)
	{
		doInferenceClassification(*context, stream, m_pData, m_pBuffers, m_pProbFloat, nCountImg, pBaseParam->m_AICommonParam.nInModelInputW, pBaseParam->m_AICommonParam.nInModelInputH, max(0, pBaseParam->m_AICommonParam.nInClassNum), m_bIsDdInput);
	}
	

	return TRUE;
}
bool AIClassifierTool::GetResult(tagAIBaseParam*pBaseParam,tagAIBaseResultParam* pBaseResultParam)
{
...
unsigned char AIClassifierTool::GetScaleVal(Img &img, float fRawX, float fRawY)
{
	int nIVal = fRawX;
	int nJVal = fRawY;
