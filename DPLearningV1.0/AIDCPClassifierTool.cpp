...
string Tchar2StringE(TCHAR *pStr)
{
...
LPCWSTR String2LpcwstrE(std::string strOrig)
{
...
void Tchar2CharE(const TCHAR * tchar, char * _char)
{
	int iLength;
	//获取字节长度  
...
void doInferenceDCPClassification(IExecutionContext& context, cudaStream_t& stream, float*input, void **buffers, float* output, int batchSize, int nInPutModelW, int nInPutModelH, int nOutPutSize, bool bIsDbInput)
{
	if (bIsDbInput)
	{
...
AIDCPClassifierTool::AIDCPClassifierTool()
{
...
int AIDCPClassifierTool::Release()
{
...
bool AIDCPClassifierTool::Init(tagAIBaseParam* pBaseParam)
{
	//1.1读取配置文件 待添加
	TCHAR exeFullPath[MAX_PATH]; // Full path
	std::string strPath = "";
	GetModuleFileName(NULL, exeFullPath, MAX_PATH); //获取带有可执行文件名路径
	strPath = Tchar2StringE(exeFullPath);
	...
	int nDLTemp = GetPrivateProfileInt(cls_modelName, L"DLTpye", 0, String2LpcwstrE(strIniPath));
	//test
	pBaseParam->m_AIBuildToolType = AIBuildToolType(nDLTemp); //dpClassifierTool;

	TCHAR szValue[MAX_PATH] = L"";
	char chInputName[MAX_PATH] = { 0 };
	GetPrivateProfileString(cls_modelName, L"InputName", L"input", szValue, MAX_PATH, String2LpcwstrE(strIniPath));
	...
	GetPrivateProfileString(cls_modelName, L"OutputName", L"output", szValue, MAX_PATH, String2LpcwstrE(strIniPath));
	...
	GetPrivateProfileString(cls_modelName, L"ModelPath", L"D:\\model\\moqie0712_s.engine", szValue, MAX_PATH, String2LpcwstrE(strIniPath));
	...
	nDLTemp = GetPrivateProfileInt(cls_modelName, L"ClassNum", 0, String2LpcwstrE(strIniPath));
	pBaseParam->m_AICommonParam.nInClassNum = nDLTemp;   //11

	int nModelW = GetPrivateProfileInt(cls_modelName, L"ModelW", 285, String2LpcwstrE(strIniPath));
	pBaseParam->m_AICommonParam.nInModelInputH = nModelW;           // 285;

	int nModelH = GetPrivateProfileInt(cls_modelName, L"ModelH", 285, String2LpcwstrE(strIniPath));
...
bool AIDCPClassifierTool::SetParam(tagAIBaseParam*pBaseParam)
{
	//2.1设置内存、显存分配
	...
	CUDA_CHECK(cudaMalloc((void**)&img_device, 1 * 6 * pBaseParam->m_AICommonParam.nInInputImgMaxW*pBaseParam->m_AICommonParam.nInInputImgMaxH));

	// Create GPU buffers on device
	CUDA_CHECK(cudaMalloc(&m_pBuffers[inputIndex],1 * 6 * pBaseParam->m_AICommonParam.nInInputImgMaxW*pBaseParam->m_AICommonParam.nInInputImgMaxH * sizeof(float)));
	CUDA_CHECK(cudaMalloc(&m_pBuffers[outputIndex],1 * nOutputSize * sizeof(float)));
	...
	m_pProbFloat = new float[1 * nOutputSize];
...
bool AIDCPClassifierTool::PreProcess(tagAIBaseParam*pBaseParam, tagAIBaseResultParam* pBaseResultParam, vector<DEF>vecInDefImg, vector<REF>vecInRefImg)
{
	pBaseParam = &m_tagAIBaseParams;
	if (pBaseParam == NULL)
	{
...
bool AIDCPClassifierTool::Execute(tagAIBaseParam*pBaseParam, tagAIBaseResultParam* pBaseResultParam)
{
	...
	if (pBaseParam->m_AIBuildToolType == dpDCPClassifierTool)
	{
		doInferenceDCPClassification(*context, stream, m_pData, m_pBuffers, m_pProbFloat, nCountImg, pBaseParam->m_AICommonParam.nInModelInputW, pBaseParam->m_AICommonParam.nInModelInputH, max(0, pBaseParam->m_AICommonParam.nInClassNum), m_bIsDdInput);
	}
	

	return TRUE;
}
bool AIDCPClassifierTool::GetResult(tagAIBaseParam*pBaseParam,tagAIBaseResultParam* pBaseResultParam)
{
...
unsigned char AIDCPClassifierTool::GetScaleVal(Img &img, float fRawX, float fRawY)
{
	int nIVal = fRawX;
	int nJVal = fRawY;
