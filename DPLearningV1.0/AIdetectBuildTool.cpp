...
string Tchar2StringD(TCHAR *pStr)
{
...
LPCWSTR String2LpcwstrD(std::string strOrig)
{
...
void Tchar2CharD(const TCHAR * tchar, char * _char)
{
	int iLength;
	//获取字节长度  
...
void doInferenceDetect(IExecutionContext& context, cudaStream_t& stream, float*input, void **buffers, float* output, int batchSize, int nInPutModelW, int nInPutModelH, int nOutPutSize, bool bIsDbInput)
{
	if (bIsDbInput)
	{
...
AIdetectBuildTool::AIdetectBuildTool()
{
	m_pAIDetectResult =NULL;
}
...
bool AIdetectBuildTool::Init(tagAIBaseParam* pBaseParam)
{
	//1.1读取配置文件 待添加
	TCHAR exeFullPath[MAX_PATH]; // Full path
	std::string strPath = "";
	GetModuleFileName(NULL, exeFullPath, MAX_PATH); //获取带有可执行文件名路径
	strPath = Tchar2StringD(exeFullPath);
	...
	int nDLTemp = GetPrivateProfileInt(L"DL", L"DLTpye", 1, String2LpcwstrD(strIniPath));
	//test
	pBaseParam->m_AIBuildToolType = AIBuildToolType(nDLTemp); //dpClassifierTool;

	TCHAR szValue[MAX_PATH] = L"";
	char chInputName[MAX_PATH] = { 0 };
	GetPrivateProfileString(L"DL", L"InputName", L"input", szValue, MAX_PATH, String2LpcwstrD(strIniPath));
	...
	GetPrivateProfileString(L"DL", L"OutputName", L"output", szValue, MAX_PATH, String2LpcwstrD(strIniPath));
	...
	GetPrivateProfileString(L"DL", L"ModelPath", L"D:\model\moqie0712_s.engine", szValue, MAX_PATH, String2LpcwstrD(strIniPath));
	...
	nDLTemp = GetPrivateProfileInt(L"DL", L"ClassNum", 0, String2LpcwstrD(strIniPath));
	pBaseParam->m_AICommonParam.nInClassNum = nDLTemp;   //11

	int nModelW = GetPrivateProfileInt(L"DL", L"ModelW", 285, String2LpcwstrD(strIniPath));
	pBaseParam->m_AICommonParam.nInModelInputH = nModelW;           // 285;

	int nModelH = GetPrivateProfileInt(L"DL", L"ModelH", 285, String2LpcwstrD(strIniPath));
...
bool AIdetectBuildTool::SetParam(tagAIBaseParam*pBaseParam)
{
	//2.1设置内存、显存分配
...
bool AIdetectBuildTool::PreProcess(tagAIBaseParam*pBaseParam, tagAIBaseResultParam* pBaseResultParam, vector<DEF>vecInDefImg, vector<REF>vecInRefImg)
{
	pBaseParam = &m_tagAIBaseParams;
	if (pBaseParam == NULL)
	{
...
bool AIdetectBuildTool::Execute(tagAIBaseParam*pBaseParam, tagAIBaseResultParam* pBaseResultParam)
{
	...
	if (pBaseParam->m_AIBuildToolType == dpDetectTool)
	{
		doInferenceDetect(*context, stream, m_pData, m_pBuffers, m_pProbFloat, nCountImg, nImgWCur, nImgHCur, nImgHCur*nImgWCur, m_bIsDdInput);
	}

	return TRUE;
}
bool AIdetectBuildTool::GetResult(tagAIBaseParam*pBaseParam,tagAIBaseResultParam* pBaseResultParam)
{
	...
	m_pAIDetectResult = (tagAIDetectResult*)pBaseResultParam;

	std::vector<std::vector<Yolo::Detection>> batch_res(pBaseResultParam->m_nBatchCur);
	for (int b = 0; b < pBaseResultParam->m_nBatchCur; b++)
	{
		auto& res = batch_res[b];
		nms(res, &m_pProbFloat[b * pBaseParam->m_AICommonParam.nInInputMaxImgSize], pBaseParam->m_AICommonParam.fProp, pBaseParam->m_AICommonParam.fNmsThrd);
	}
	m_pAIDetectResult->m_vecDetBox.clear();
	m_pAIDetectResult->m_vecDetBox.resize(pBaseResultParam->m_nBatchCur);
	for (int b = 0; b < pBaseResultParam->m_nBatchCur; b++)
	{
		auto& res = batch_res[b];
		/*int imgcols = vecImg[b].nWidth;
		int imgrows = vecImg[b].nHeight;*/
		int imgcols = pBaseResultParam->m_nImgWCur;
		int imgrows = pBaseResultParam->m_nImgHCur;
		for (size_t j = 0; j < res.size(); j++)
		{
			cv::Rect r = get_rect(imgcols, imgrows, res[j].bbox, pBaseParam->m_AICommonParam.nInModelInputW, pBaseParam->m_AICommonParam.nInModelInputH);// by ls 标记 20220708 
			DetBox dbox;
			dbox.fClassID = res[j].class_id;
			dbox.fConfScore = res[j].conf;
			dbox.fBBox[0] = r.tl().x;
			if (dbox.fBBox[0] < 0)
			{
				dbox.fBBox[0] = 0;
			...
			dbox.fBBox[2] = r.br().x;
			if (dbox.fBBox[2] > imgcols)
			{
				dbox.fBBox[2] = imgcols;
			}
			dbox.fBBox[3] = r.br().y;
			if (dbox.fBBox[3] > imgrows)
			{
				dbox.fBBox[3] = imgrows;
			}
			m_pAIDetectResult->m_vecDetBox[b].push_back(dbox);
		}
	}
	//
